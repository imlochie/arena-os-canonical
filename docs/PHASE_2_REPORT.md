...
| `npm run test:compose` | passed — 4 real-stack Playwright tests against the canonical Docker Compose topology |
...
Its four passing tests also established the exercised authorization and failure behavior: project/private-media isolation and collaborator roles, corrupt-audio rejection, a terminal real-Demucs failure that did not produce stems, and waveform recovery after injected storage-boundary faults plus an active-worker restart. The recovery scenario verifies retry/idempotency, partial-artifact cleanup, and duplicate durable-artifact protection. This is runtime evidence for the tested Compose path, not a general claim that every infrastructure-failure mode has been exercised.

## Remaining verification boundaries

- CUDA Demucs remains unverified and must be tested only on actual NVIDIA-capable infrastructure.
- The completed gate does not establish production-cloud storage behavior, long-duration workload behavior, backup/restore, monitoring, or operational incident handling.
- Repository CI now invokes the same Compose gate from a clean checkout. A green GitHub Actions workflow is additional CI evidence, not a claim that production operations or CUDA have been verified.

## Follow-up work

1. Observe and maintain the repository-level Compose CI gate, including its failure diagnostics and cleanup behavior.
