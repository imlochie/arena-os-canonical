...
At `a1c8e37c3623a72dc6b9a5aee2bd10eabfe76cb5`, a Docker-capable host ran `npm run test:compose` successfully against the full seven-test suite: `7 passed (3.1m)`. The verified local Docker Compose run exercised the Phase 4 publication/moderation path and tore down Compose resources cleanly. This is local Compose evidence, not a GitHub Actions result: Actions runner allocation is presently blocked by an external billing limitation.
