...
export function StemMixer({ stems }: { stems: Stem[] }) {
  ...
  return <section className="mixer" aria-label="Stem mixer"><div className="transport"><button onClick={() => void toggle()}>{playing ? "Pause" : "Play all"}</button><input aria-label="Seek stems" type="range" min="0" max={duration || 1} value={position} step="0.01" onChange={(event) => setAllPosition(Number(event.target.value))} /><span>{clock(position)} / {clock(duration)}</span></div>{stems.map((stem, index) => <article className="stem" key={stem.id}><div><b>{stem.stemType}</b><small>{stem.format.toUpperCase()} · {stem.sampleRate} Hz · {stem.channels} ch</small></div><div><input aria-label={`${stem.stemType} volume`} type="range" min="0" max="1" step="0.01" value={volume[stem.id] ?? 1} onChange={(event) => updateVolume(stem.id, Number(event.target.value))} /><audio ref={(node) => { refs.current[stem.id] = node; }} src={`/api/assets/${stem.id}`} preload="metadata" onTimeUpdate={(event) => { if (index === 0) setPosition(event.currentTarget.currentTime); }} onEnded={() => { if (index === 0) setPlaying(false); }} /></div><button className="button secondary" onClick={() => toggleMute(stem.id)}>{muted[stem.id] ? "Unmute" : "Mute"}</button></article>)}</section>;
}
