...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const signed = await storage.createDownloadUrl(asset.storageKey, 60);
    ...
    if (range) {
      ...
      return new Response(Readable.toWeb(createReadStream(localPath, { start, end })) as ReadableStream, { status: 206, headers: { "Content-Type": contentType, "Content-Length": String(end - start + 1), "Content-Range": `bytes ${start}-${end}/${file.size}`, "Accept-Ranges": "bytes", "Cache-Control": "private, no-store" } });
    }
    return new Response(Readable.toWeb(createReadStream(localPath)) as ReadableStream, { headers: { "Content-Type": contentType, "Content-Length": String(file.size), "Accept-Ranges": "bytes", "Cache-Control": "private, no-store" } });
