  ...
  return new Promise<{ stdout: string; stderr: string }>(
    (resolvePromise, reject) => {
      ...
      const root =
        ...
        (existsSync(resolve(current, "services/separation/separate.py"))
          ? current
          : resolve(current, "../.."));
      ...
      child.once("close", (code) => {
        if (code === 0) resolvePromise({ stdout, stderr });
          ...
          reject(
            new Error(
              `Demucs process failed (exit ${code}): ${(stderr || stdout).slice(-2000)}`,
            ),
          );
      });
    },
...
async function validateStems(
  outputDirectory: string,
): Promise<ValidatedStem[]> {
  ...
  for (const stemType of STEMS) {
    ...
    results.push({
      ...
      path,
      metadata,
      checksumSha256: await checksumFile(path),
    });
  }
  return results;
}
...
export async function processSeparation(
  ...
  const [job] = await db
    .select()
    .from(processingJobs)
    .where(eq(processingJobs.id, payload.processingJobId))
    .limit(1);
  ...
  if (
    ...
    await updateJob(job.id, {
      ...
      errorMessage:
        "Separation queue payload does not match its durable job target.",
      completedAt: new Date(),
    });
    throw new Error(
      "Separation payload does not match its durable job target.",
    );
  }
  ...
  try {
    ...
    const { stdout } = await runPython(
      [
        ...
        payload.model,
        "--device",
        payload.requestedDevice,
      ],
      tempDirectory,
    );
    const result = JSON.parse(
      stdout.trim().split("\n").at(-1) ?? "{}",
    ) as SeparationEngineMetadata;
    if (!result.resolvedDevice || result.engine !== "demucs")
      throw new Error("Demucs adapter returned invalid engine metadata.");
