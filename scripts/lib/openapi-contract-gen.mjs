/**
 * openapi-contract-gen.mjs — shared OpenAPI-subset contract generator.
 *
 * Single machinery, two sanctioned seams (see the thin drivers):
 *
 *   scripts/generate-archive-assistant-contract.mjs   six-op read-only bridge
 *   scripts/generate-personalisation-contract.mjs     seventh read (Gate 6)
 *
 * The generator is deliberately dependency-free: it carries a small YAML
 * reader that covers the OpenAPI authoring style used upstream (block
 * maps/sequences, inline flow collections, quoted and plain scalars,
 * literal block scalars).
 *
 * Drivers supply: the operation allow-list, the output directory, the
 * export names, the doc paragraphs embedded in the generated files, and the
 * fetch defaults (repo/ref/source-label/source-ref). Emitted bytes are a
 * pure function of (spec content, seam config) — no timestamps — which is
 * what makes each driver's --check mode meaningful.
 */
...
function extractSubset(doc, operations, boundaryNote) {
  ...
  if (missing.length) {
    throw new Error(
      `The upstream spec does not expose ${boundaryNote}:\n  - `
      + missing.join("\n  - ")
      + "\nUse a ref that contains it (see header note in the generating script).",
    );
  }
...
function emitContractTs(meta, subset, seam) {
  return `${header(meta, seam)}
${seam.contractDoc}

export const ${seam.exportNames.meta} = ${JSON.stringify(meta, null, 2)};

export const ${seam.exportNames.operations} = ${JSON.stringify(subset.operations, null, 2)};

export const ${seam.exportNames.schemas} = ${JSON.stringify(subset.schemas, null, 2)};
`;
}

function header(meta, seam) {
  return `/**
 * -----------------------------------------------------------------------------
...
function parseArgs(argv, defaults) {
  const args = {
    ...
    ref: defaults.ref,
    check: false,
    sourceLabel: defaults.sourceLabel ?? null,
    sourceRef: defaults.sourceRef ?? null,
  };
...
/**
 * Run one seam end to end: fetch spec → extract the operation allow-list +
 * transitive schema closure → emit → write (or --check against committed).
 *
 * seam = {
 *   operations,          // [{ operationId, method, path }] — the ONLY
 *                        // operations ever generated for this seam
 *   boundaryNote,        // name of the boundary in error messages
 *   outDir,              // where generated files live
 *   exportNames: { meta, operations, schemas },
 *   npmCommand,          // e.g. "generate:archive-contract"
 *   typesDoc, contractDoc,   // paragraphs embedded verbatim in output
 *   defaults: { repo, ref, sourceLabel?, sourceRef? },
 *   checkName,           // e.g. "Archive Assistant contract"
 * }
 */
export async function runGenerator(seam, argv = process.argv.slice(2)) {
  const args = parseArgs(argv, seam.defaults);
  ...
  const subset = extractSubset(doc, seam.operations, seam.boundaryNote);
  // Note: no timestamp is recorded — output is byte-for-byte reproducible
  // from (spec content, seam config), which is what makes the driver's
  // --check mode meaningful.
  const meta = {
  ...
  const files = {
    "contract.ts": emitContractTs(meta, subset, seam),
    "types.ts": emitTypesTs(meta, subset.schemas, seam),
  };

  const outDir = join(REPO_ROOT, seam.outDir);
  if (args.check) {
    const stale = [];
    ...
    if (stale.length) {
      console.error(`${seam.checkName} is stale: ${stale.join(", ")}`);
      console.error(`Run: npm run ${seam.npmCommand}`);
      process.exit(1);
    }
    console.log(`${seam.checkName} is up to date.`);
  } else {
    mkdirSync(outDir, { recursive: true });
    for (const [name, content] of Object.entries(files)) {
      writeFileSync(join(outDir, name), content);
      console.log(`wrote ${seam.outDir}/${name} (${content.length} bytes)`);
    }
